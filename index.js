...
class HouseSerivce {
    static url = 'https://ancient-taiga-31359.herokuapp.com/api/house';
...
class DOMManager {
    ...
    static getAllHouses() {
        HouseSerivce.getAllHouses().then(houses => this.render(houses));
    }

    static createHouse(name) {
        HouseSerivce.createHouse(new HouseSerivce(name))
          .then(() => {
            return HouseSerivce.getAllHouses();
          })
          .then((houses) => this.render(houses));
    }

    static deleteHouse(id) {
        HouseSerivce.deleteHouse(id)
        .then(() => {
            return HouseSerivce.getAllHouses();
        })
        .then((houses) => this.render(houses));
    ...
    static addroom(id) {
        for (let house of this.houses) {
            if (house._id == id) {
                ...
                HouseSerivce.updateHouse(house)
                    .then(() => {
                        return HouseSerivce.getAllHouses();
                    })
                    .then((houses) => this.render(houses));
